...
function Discover({searchText}:{searchText:string}) {
  ...
  return (
    <>
        ...
        {data&&data.data.map((data,index:number)=><Card key={index} artists={data.artist.name} title={data.album.title} artistImg={data.artist.picture_medium}  song={data.preview}/>)}
